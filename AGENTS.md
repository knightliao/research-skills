...
`examples/usage.md` 回答“用户应该怎么问”，不用于展示完整高质量输出。高质量输出、反面输出和转换结果应继续使用独立示例文件，并由 `SKILL.md` 按需加载。

每个 `SKILL.md` 必须包含指向 `examples/usage.md` 的有效相对链接，但不应复制大量完整示例。只在用户询问如何使用、要求可复制输入、已选择 Skill 但缺少必要材料，或需要选择工作模式时读取该文件，不要在每次执行时默认加载。

使用示例必须与当前已经定义和实现的能力一致，不得承诺未实现的模式、输入、输出或外部操作。示例不得写入真实凭据、私人路径、内部地址或敏感数据；默认使用 `<文件路径>`、`<任务书>`、`<公司名称>` 等占位符。

新增 Skill 时必须同步更新根 README 的快速使用入口。`agents/openai.yaml` 仍按实际宿主集成需要提供，不是所有 Skill 的强制文件。

以上要求是当前仓库的产品化规范，不得为此修改 `skill_framework/`，也不得将其上升为所有外部 Agent Skill 的通用标准。
