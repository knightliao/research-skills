...
头像检索是“文章事实不得联网补写”的唯一素材获取例外，只能用于定位和确认配图。主人公无法唯一确认、找不到可靠清晰头像、来源或使用权限不清，或微修工具不可用时，不得改用 AI 生成的相似人物冒充本人；运行 `fail` 记录原因，保留 incomplete 包，并请求用户确认人物、提供官方链接或提供有权使用的头像。
